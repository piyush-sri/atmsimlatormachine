...
public class PinChange extends JFrame implements ActionListener
{
 ...
 int reply=JOptionPane.showConfirmDialog(null,"Do you want continue?","ATM Time Warning",JOptionPane.YES_NO_OPTION);
		...
		else if(ae.getSource()==b1)
		{
         ...
         if(((p1.getPassword()).equals(""))&&((p2.getPassword()).equals(""))&&((p3.getPassword() ).equals("")))
	       {
 JOptionPane.showMessageDialog(this,"Please enter old PIN no 1 times and new PIN no 2 times !","Warning",JOptionPane.WARNING_MESSAGE);
	        ...
	        else if(!(Arrays.equals(p2.getPassword(), p3.getPassword())))
	    {
...
ps=con.prepareStatement("select * from accountdetail where atmno='"+atno+"' and accno='"+acno+"' and acctype='"+actype+ " and pinno="+ pno);
	       rs=ps.executeQuery();
		         while(rs.next())
	             {
	             	foundrec=1;
